...
\section{Results}
\subsection{W and variance fields, plus tau_c}
For each time step and coarsening scale, a plot is drawn of the vertical velocity field plus the normalized variance 

\subsection{Cloud size and m/p distribution and RDF}
For each time step the cloud size and m/p distribution is evaluated as an ensemble average over the whole analysis domain. Figure \ref{fig:} shows one this distribution for one time step (12UTC). Plots for all time steps can be found at ...

\subsection{Variance scatter plots}
Figure shows a variance scatter plot similar to Fig 4 in CC06 for one time step. Each dot represents one coarse grained box. The dashed line represents the theoretical scaling. 

\subsection

\section{Discussion}
\subsection{Variance scatter plots: Average value for each neighborhood size}

Question: Do I need to have one average value for the variance for one scale. If yes, how do I do that. A normal mean will be heavily affected by outliers. 
